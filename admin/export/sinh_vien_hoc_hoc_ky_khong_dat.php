<?php 
	...
	function el_sinh_vien_hoc_khoa_hoc($kh){
		...
		$conn = $ketnoi->ketnoi();
		$query = "SELECT DISTINCT sv.IDSV,sv.HOSV, sv.TENSV,sv.GIOITINH,sv.NGAYSINH,sv.MAIL, sv.SDT, sv.DIACHI, d.DIEM,kh.TENKH FROM sinhvien sv, khoahoc kh, sv_dk_khoahoc sd, diemkhoahoc d WHERE sv.IDSV = sd.IDSV AND kh.IDKH = sd.IDKH AND sd.XACNHAN = b'0' AND kh.IDKH = d.IDKH AND sd.IDSV = d.IDSV AND d.DIEM < '5' AND kh.IDKH = '$kh' AND sv.TRANGTHAI = b'0' ORDER BY sv.TENSV ASC";
		...
		return $result;
			...
			$sheet->setCellValue('A1',"DANH SÁCH SINH VIÊN KHÔNG ĐẠT KHÓA HỌC $tenkh");
			// Đường viền
